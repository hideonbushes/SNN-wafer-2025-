"""04) Original + Spike-driven self-attention hybrid architecture."""
import torch
...
class Wafer2SpikeAttention(nn.Module):
    def __init__(self, numClasses, dropout_fc, spike_ts, device, params):
        ...
        self.conv0 = CurrentBasedGLIF(nn.Conv2d(1, 64, 7, stride=1, bias=True), pseudo, params)
        self.conv1 = CurrentBasedGLIF(nn.Conv2d(64, 64, 7, stride=2, bias=True), pseudo, params)
        self.conv2 = CurrentBasedGLIF(nn.Conv2d(64, 64, 7, stride=2, bias=True), pseudo, params)
    ...
    def forward(self, x):
        ...
        for t in range(self.spike_ts):
            ...
            tok = h2.flatten(2).transpose(1, 2)  # (B, 9, 64)
...
if __name__ == "__main__":
    ...
    training(network=CurrentBasedSNNAttn, params=[0.05, 0.10, 0.08, 0.30], dataloaders=dataloaders)
