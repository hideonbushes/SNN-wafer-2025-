# -*- coding: utf-8 -*-
"""Original Wafer2Spike + rate encoding preprocessing."""
...
def _rate_encode_image(image_norm: np.ndarray, num_steps: int = 10) -> np.ndarray:
    """Convert normalized intensity map to rate-coded map (mean firing probability)."""
...
def build_dataloaders(data_path="/content/drive/MyDrive/WM-811k/LSWMD.pkl", batch_size=256, rate_steps=10):
    ...
    def preprocess_images(arrays):
        ...
        for im in arrays:
            ...
            im_e = _rate_encode_image(im_r, num_steps=rate_steps)
            out.append(im_e)
